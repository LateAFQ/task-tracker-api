...
from src.api.v1.handlers.auth.login import LoginHandler, LoginQuery
from src.api.v1.handlers.auth.logout import LogoutHandler, LogoutQuery
from src.api.v1.handlers.auth.refresh import RefreshTokenHandler, RefresTokenQuery
...
def setup_mediator(mediator: MediatorImpl, **dependencies: Any) -> None:
    mediator.register(dtos.CreateUser, user.CreateUserHandler)
    mediator.register(dtos.DeleteUser, user.DeleteUserHandler)
    mediator.register(dtos.SelectUser, user.SelectUserHandler)
    mediator.register(dtos.UpdateUserQuery, user.UpdateUserHandler)
    mediator.register(LoginQuery, LoginHandler)
    mediator.register(auth.PermissionQuery, auth.PermissionHandler)
    mediator.register(RefresTokenQuery, RefreshTokenHandler)
    mediator.register(LogoutQuery, LogoutHandler)
